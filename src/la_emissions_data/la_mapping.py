...
from data_common.charting import Chart
...
lower_tier = top_level / "data" / "resources" / "LocalAuthorities-lowertier.gpkg"
assert lower_tier.exists()
...
def get_label_and_offset(gdf, label, x, y, switch=False) -> pd.DataFrame:
    ...
    return pd.concat([gdf, item])  # type:ignore
...
def lower_tier_base_layer() -> Chart:
    ...
    background_layer = geopandas.read_file(lower_tier, layer="7 Background").to_crs(
        epsg=4326
    )
    groups_layer = geopandas.read_file(lower_tier, layer="2 Groups").to_crs(epsg=4326)

    base = (
        Chart(gdf_to_data(background_layer))
        .mark_geoshape(stroke="black", fill="lightgrey", strokeWidth=0)  # type: ignore
        .encode()
    )

    groups = (
        Chart(gdf_to_data(groups_layer))
        .mark_geoshape(stroke="black", fill=None, strokeWidth=1)  # type: ignore
        .encode()
    )
...
def lower_tier_label_layer() -> Chart:
    ...
    labels_layer = geopandas.read_file(lower_tier, layer="1 Group labels").to_crs(
        epsg=4326
    )
    ...
    labels_layer = get_label_and_offset(labels_layer, "West of England", 0, +0.00001)
    labels_layer = get_label_and_offset(
    ...
    text_props = {"baseline": "middle", "font": "Source Sans Pro"}

    labels_left = Chart(gdf_to_data(labels_layer_left)).mark_text(  # type: ignore
        align="left", **text_props  # type: ignore
    )
    labels_right = Chart(gdf_to_data(labels_layer_right)).mark_text(  # type: ignore
        align="right", **text_props  # type: ignore
    )
...
def lower_tier_layer() -> geopandas.GeoDataFrame:
    ...
    """
    gdp = geopandas.read_file(lower_tier, layer="6 LTLA-2021").to_crs(epsg=4326)
...
def lower_tier_sandwich(tier_layer: Chart) -> Chart:
    ...
    return base_layer + tier_layer + label_layer  # type: ignore
